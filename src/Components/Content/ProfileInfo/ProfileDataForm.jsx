...
export const ProfileDataForm = ({ handleSubmit, profile, error }) => {
    ...
    return <form onSubmit={handleSubmit} className={classes.aboutMe}>
        ...
        <div className={classes.btnSaveDiv}>
            <ButtonStyled type='submit' margin='10px'>
        ...
        <div className={classes.descriptionBlock}>
            <h4 className={classes.edit}>Looking for a job:</h4>
            <div className={classes.checkboxBlock}><Field className={classes.checkbox} name={'checkbox'}
                component={Input} type={'checkbox'} /></div>
            <div><h4 className={classes.edit}>My professional
                skills</h4>: {createField('lookingForAJobDescription', 'My professional skills', Textarea)}</div>
            ...
            <div>
                ...
                {Object.keys(profile.contacts).map(key => {
                    return <>
                        <b key={key}
                            className={classes.contactsTitle}>{key}:</b>{createField("contacts." + key, key, Input)}
                    </>
                })}
            </div>
        </div>
